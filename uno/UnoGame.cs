...
public partial class UnoGame : Node
{
	...
	public void _on_card_submit()
	{
		Card submitted_card = hand[GetNode<OptionButton>("%Hand_Options").Selected];

		if(submitted_card.colour == discard[discard.Count - 1].colour)
        {
            Discard(GetNode<OptionButton>("%Hand_Options").Selected);

			GetNode<Label>("%Notice").Text = "";
        }
		else if(submitted_card.number == discard[discard.Count - 1].number)
        {
            Discard(GetNode<OptionButton>("%Hand_Options").Selected);
        ...
        {
            GetNode<Label>("%Notice").Text = "Frick u";
        }
